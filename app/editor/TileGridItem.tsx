...
const TileGridItem: React.FC<TileGridItemProps> = ({ sheetName, spriteName, selected, onClick }) => {
    ...
    const Tile = () => {
        ...
        const spriteData = sprites.spriteSheetDataSet[sheetName].sprites[spriteName]
        if (!spriteData) {
            console.error(`Sprite ${spriteName} not found in sheet ${sheetName}`)
            return <div>Error: Sprite not found</div>
        }
